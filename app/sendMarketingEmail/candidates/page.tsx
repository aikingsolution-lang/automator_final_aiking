...
interface Recipient {
    email: string;
    name: string;
    [key: string]: string; // Allow any additional fields from CSV
}
...
export default function MarketingEmailPage() {
    ...
    useEffect(() => {
        const checkBackendConnectivity = async () => {
            try {
                const response = await fetch('https://marketing-email-for-candidates.onrender.com/', {
                    method: 'GET',
                    headers: {
    ...
    // Parse CSV file
    const parseCSV = (file: File): Promise<{ recipients: Recipient[], columns: string[] }> => {
                ...
                try {
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        if (email && isValidEmail(email)) {
                            // Create recipient object with all column values
                            const recipient: any = { email };

                            // Add all column values to recipient object
                    ...
                    resolve({ recipients, columns: headers });
                } catch (error) {
    ...
    const replacePlaceholders = (content: string, recipient: Recipient): string => {
        let result = content;

        // Replace all column placeholders with actual values
        csvColumns.forEach(column => {
            const placeholder = `{${column}}`;
            const value = recipient[column] || '';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const { recipients: parsedRecipients, columns } = await parseCSV(file);
            setRecipients(parsedRecipients);
            setCsvColumns(columns);
            setUploadStatus(`Successfully loaded ${parsedRecipients.length} recipients from CSV with ${columns.length} columns.`);
        } catch (error) {
    ...
    const checkOAuth2Auth = async (email: string) => {
        try {
            ...
            const response = await fetch('https://marketing-email-for-candidates.onrender.com/check-auth', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleOAuth2Auth = (email: string) => {
        const authUrl = `https://marketing-email-for-candidates.onrender.com/auth/google?state=candidates`;
    ...
    const checkCampaignStatus = async (id: string) => {
        try {
            ...
            const response = await fetch(`https://marketing-email-for-candidates.onrender.com/campaign-status/${id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const loadCampaignHistory = async () => {
        try {
            ...
            const response = await fetch('https://marketing-email-for-candidates.onrender.com/campaigns', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleSendEmails = async () => {
        ...
        try {
            ...
            const response = await fetch('https://marketing-email-for-candidates.onrender.com/start-campaign', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    csvColumns, // Send column information to backend
                    usePlaceholders: true // Flag to indicate placeholder replacement should be used
                }),
            });
            ...
            if (response.ok) {
                ...
                if (response.status === 401 || response.status === 403) {
                    ...
                    console.error('Campaign start failed:', result);
            ...
            <div className="max-w-4xl mx-auto">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                            ...
                            <div className="space-y-4">
                                ...
                                {uploadStatus && (
                                    <div className={`p-4 rounded-lg text-sm ${uploadStatus.includes('Successfully') || uploadStatus.includes('completed')
                            ...
                            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                                <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                                    <svg className="w-5 h-5 mr-2 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                    </svg>
                                ...
                                {csvColumns.length > 0 && (
                                    <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                                        <p className="text-blue-400 text-sm font-medium mb-2">Available CSV Columns:</p>
                                        ...
                                        <p className="text-gray-400 text-xs mt-2">
                                            Use these as placeholders: {'{'}{'column_name'}{'}'} in your email content
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="max-h-60 overflow-y-auto space-y-2">
                                    {recipients.slice(0, 10).map((recipient, index) => (
                                        <div key={index} className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
                                            <div>
                                                <p className="text-white font-medium">{recipient.name || recipient.email.split('@')[0]}</p>
                                                <p className="text-gray-400 text-sm">{recipient.email}</p>
                                                {csvColumns.length > 2 && (
                                                    <p className="text-gray-500 text-xs">
                                                        {csvColumns.slice(0, 3).map(col => `${col}: ${recipient[col] || 'N/A'}`).join(', ')}
                                                        {csvColumns.length > 3 && '...'}
                                                    </p>
                                                )}
                                            </div>
                        ...
                        <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Input
                                        type="text"
                                        placeholder="Exciting opportunity for - {name}!"
                                ...
                                {/* Placeholder Preview */}
                                {recipients.length > 0 && csvColumns.length > 0 && (
                                    <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                                        <h4 className="text-yellow-400 font-medium mb-2">Placeholder Preview (First Recipient):</h4>
                                        <div className="space-y-2 text-sm">
                                            <div>
                                                <span className="text-gray-400">Subject: </span>
                                                <span className="text-white">
                                                    {replacePlaceholders(emailCampaign.subject, recipients[0])}
                                                </span>
                                            </div>
                                            <div>
                                                <span className="text-gray-400">Body Preview: </span>
                                                <span className="text-white">
                                                    {replacePlaceholders(emailCampaign.body, recipients[0]).substring(0, 100)}
                                                    {replacePlaceholders(emailCampaign.body, recipients[0]).length > 100 && '...'}
                                                </span>
                                            </div>
                                            {emailCampaign.footer && (
                                                <div>
                                                    <span className="text-gray-400">Footer: </span>
                                                    <span className="text-white">
                                                        {replacePlaceholders(emailCampaign.footer, recipients[0])}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    {campaignStatus ? (
                                        <>
                                            <div className="flex justify-between items-center">
                                                ...
                                                <span className={`font-medium ${campaignStatus.status === 'completed' ? 'text-green-400' :
                            ...
                            {campaignHistory.map((campaign) => (
                                <div key={campaign.id} className="bg-white/5 rounded-lg p-4 border border-white/10">
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${campaign.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                ...
                <div className="mt-8 bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <h4 className="text-white font-medium mb-2">CSV Format & Placeholders:</h4>
                            <ul className="text-gray-300 space-y-1 text-sm">
                                <li>• <strong>email</strong> - Recipient's email address (required)</li>
                                <li>• <strong>name</strong> - Recipient's name (optional)</li>
                                <li>• <strong>Any other columns</strong> - Will be available as placeholders</li>
                            </ul>
                            <div className="bg-gray-800 rounded-lg p-3 text-sm font-mono text-gray-300 mt-2">
                                email,name,company,position<br />
                                john@example.com,John Doe,ABC Corp,Developer<br />
                                jane@example.com,Jane Smith,XYZ Inc,Manager<br />
                                bob@example.com,Bob Johnson,Tech Co,Designer
                            </div>
                            <div className="bg-blue-800 rounded-lg p-3 text-sm font-mono text-gray-300 mt-2">
                                <p className="text-blue-300 mb-1">Use placeholders in your email:</p>
                                Subject: "Hello {'{name}'}, opportunity at {'{company}'}"<br />
                                Body: "Hi {'{name}'}, we have a {'{position}'} role at {'{company}'}..."
                            </div>
                        </div>
                    </div>
                </div>
