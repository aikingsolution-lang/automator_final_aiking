...
import { ref, onValue, update, getDatabase, ref as dbRef, get } from "firebase/database";
...
const RecruiterDetailsPage = () => {
  ...
  useEffect(() => {
    if (!isAdmin) return;
    const recruitersRef = ref(database, "recruiters_marketing_data");
    const unsubscribe = onValue(recruitersRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // Convert object to array and filter isDownloaded === false
        const arr: Recruiter[] = Object.entries(data)
          .filter(([_, r]) => r && (r as Recruiter).isDownloaded === false)
          .map(([key, r]) => ({
            name: (r as any).name || "",
            email: (r as Recruiter).email,
            isDownloaded: (r as Recruiter).isDownloaded,
            key
          }));
        setRecruiters(arr);
      } else {
  ...
  const handleDownload = async () => {
    ...
    recruiters.forEach((r) => {
      const key = r.key;
      console.log("key", key);
      updates[`recruiters_marketing_data/${key}/isDownloaded`] = true;
    });
  ...
  return (
    <main className="flex items-center justify-center min-h-screen bg-gradient-to-b from-[#11011E] via-[#35013E] to-[#11011E] p-6 relative overflow-hidden">
      ...
      {/* Glassmorphism Card */}
      <section className="relative w-full max-w-4xl bg-[rgba(255,255,255,0.02)] rounded-2xl shadow-2xl border border-[rgba(255,255,255,0.07)] p-10 z-10 backdrop-blur-md animate-fade-in-up">
        <button
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-center text-[#0FAE96] mb-4 font-raleway drop-shadow-lg">Recruiter Details</h1>
        <p className="text-[#B6B6B6] text-center mb-8 text-lg">All recruiter details coming from JobForm Automator will be displayed here.</p>
        {authLoading ? (
          <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#0FAE96", fontSize: 20, animation: "pulse 1s infinite" }}>Loading...</span></div>
        ) : accessDenied ? (
          ...
          <div className="bg-[rgba(15,174,150,0.08)] border border-[#0FAE96]/20 rounded-lg p-6 text-center">
            <span className="text-[#ECF1F0] text-base md:text-lg font-medium">No recruiter data to display.</span>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 gap-4 mb-6">
              {recruiters.map((r) => (
                <div key={r.key} className="bg-[rgba(15,174,150,0.08)] border border-[#0FAE96]/20 rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between shadow">
                  <div className="flex-1">
                    <div className="flex flex-col gap-2">
                      <div className="text-[#0FAE96] font-semibold">Name: <span className="text-[#ECF1F0]">{r.name}</span></div>
                      <div className="text-[#0FAE96] font-semibold">Email: <span className="text-[#ECF1F0]">{r.email}</span></div>
                    </div>
                  </div>
