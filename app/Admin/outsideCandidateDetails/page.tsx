...
const OutsideCandidateDetailsPage = () => {
    ...
    useEffect(() => {
        if (!isAdmin) return;
        const candidatesRef = ref(database, "shortlisted_marketing_data");
        const unsubscribe = onValue(candidatesRef, (snapshot) => {
            const data = snapshot.val();
            if (data) {
                // Convert object to array and filter isDownload === false
                const arr: Candidate[] = Object.entries(data)
                    .filter(([_, c]) => c && (c as Candidate).isDownload === false)
                    .map(([key, c]) => ({ ...(c as Candidate), key }));
                setCandidates(arr);
            } else {
                setCandidates([]);
            }
    ...
    const handleDownload = async () => {
        setDownloading(true);
        // Prepare CSV (name, email, and phone fields)
        const csv = Papa.unparse(
            candidates.map(({ name, email, phone }) => ({ name, email, phone: phone || "" }))
        );
        // Download CSV
        const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
        saveAs(blob, "outside_candidates.csv");

        // Update isDownload in Firebase
        const updates: Record<string, boolean> = {};
        candidates.forEach((c) => {
            updates[`shortlisted_marketing_data/${c.key}/isDownload`] = true;
        });
    ...
    if (authLoading) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#0FAE96", fontSize: 20, animation: "pulse 1s infinite" }}>Loading...</span></div>;
    ...
    return (
        <main className="flex items-center justify-center min-h-screen bg-gradient-to-b from-[#11011E] via-[#35013E] to-[#11011E] p-6 relative overflow-hidden">
            ...
            <div className="absolute -bottom-16 -right-16 w-80 h-80 bg-[#0FAE96] rounded-full blur-3xl opacity-20 z-0"></div>
            {/* Glassmorphism Card */}
            <section className="relative w-full max-w-2xl bg-[rgba(255,255,255,0.02)] rounded-2xl shadow-2xl border border-[rgba(255,255,255,0.07)] p-10 z-10 backdrop-blur-md animate-fade-in-up">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-center text-[#0FAE96] mb-4 font-raleway drop-shadow-lg">Outside Candidate Details</h1>
                <p className="text-[#B6B6B6] text-center mb-8 text-lg">All outside candidate details (from HR/Resume Shortlisting) will be displayed here.</p>
                {loading ? (
                    ...
                    <div className="bg-[rgba(15,174,150,0.08)] border border-[#0FAE96]/20 rounded-lg p-6 text-center">
                        <span className="text-[#ECF1F0] text-base md:text-lg font-medium">No candidate data to display.</span>
                    </div>
                ) : (
                    <>
                            ...
                            {candidates.map((c) => (
                                ...
                                <div key={c.key} className="bg-[rgba(15,174,150,0.08)] border border-[#0FAE96]/20 rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between shadow">
                                    <div className="flex flex-col">
                                        <div className="text-[#0FAE96] font-semibold">
                                            Name: <span className="text-[#ECF1F0]">{c.name}</span>
                                        </div>
                                        <div className="text-[#0FAE96] font-semibold mt-1">
                                            Email: <span className="text-[#ECF1F0]">{c.email}</span>
                                        </div>
                                        {c.phone && (
                                            <div className="text-[#0FAE96] font-semibold mt-1">
                                                Phone: <span className="text-[#ECF1F0]">{c.phone}</span>
                                            </div>
                                        )}
                                    </div>
                                    <div className="mt-2 md:mt-0 md:ml-8 flex items-center">
...
export default OutsideCandidateDetailsPage;
