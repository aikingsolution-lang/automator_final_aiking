...
const CandidateDetailsPage = () => {
  ...
  useEffect(() => {
    if (!isAdmin) return;
    const candidatesRef = ref(database, "candidates_marketing_data");
    const unsubscribe = onValue(candidatesRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // Convert object to array and filter isDownloaded === false
        const arr: Candidate[] = Object.entries(data)
          .filter(([_, c]) => c && (c as Candidate).isDownloaded === false)
          .map(([key, c]) => ({
            name: (c as any).name || "", // Extract name, fallback to empty string
            email: (c as Candidate).email,
            contact: (c as Candidate).contact,
            isDownloaded: (c as Candidate).isDownloaded,
            key
          }));
        setCandidates(arr);
      } else {
  ...
  const handleDownload = async () => {
    ...
    const csv = Papa.unparse(
      candidates.map(({ name, email, contact }) => ({ name, email, contact }))
    );
    // Download CSV
    ...
    candidates.forEach((c) => {
      const key = c.key;
      console.log("key",key);
      updates[`candidates_marketing_data/${key}/isDownloaded`] = true;
    });
  ...
  return (
    <main className="flex items-center justify-center min-h-screen bg-gradient-to-b from-[#11011E] via-[#35013E] to-[#11011E] p-6 relative overflow-hidden">
      ...
      <section className="relative w-full max-w-2xl bg-[rgba(255,255,255,0.02)] rounded-2xl shadow-2xl border border-[rgba(255,255,255,0.07)] p-10 z-10 backdrop-blur-md animate-fade-in-up">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-center text-[#0FAE96] mb-4 font-raleway drop-shadow-lg">Candidate Details</h1>
        <p className="text-[#B6B6B6] text-center mb-8 text-lg">All candidate details coming through JobForm Automator will be displayed here.</p>
        {authLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 mb-6">
              {candidates.map((c) => (
                <div key={c.key} className="bg-[rgba(15,174,150,0.08)] border border-[#0FAE96]/20 rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between shadow">
                  <div>
                    <div className="text-[#0FAE96] font-semibold">Name: <span className="text-[#ECF1F0]">{c.name}</span></div>
                    <div className="text-[#0FAE96] font-semibold">Email: <span className="text-[#ECF1F0]">{c.email}</span></div>
                    <div className="text-[#0FAE96] font-semibold">Phone: <span className="text-[#ECF1F0]">{c.contact}</span></div>
                  </div>
